...
sed -i "s#Set Your IP Address to continue.#设置你的IP地址以继续#g" start_firsttime_init.sh
...
sed -i "s#YOUR IP:#你的IP地址:#g" start_ztncui.sh
sed -i "s#YOUR DOMAIN:#你的域名:#g" start_ztncui.sh
sed -i "s#ZT1 AuthToken is not found... Wait for ZT1 to start...#未找到 ZeroTier One AuthToken... 等待 ZeroTier One 启动...#g" start_ztncui.sh
sed -i "s#ZTNCUI ENV CONFIGURATION:#ZTNCUI 环境配置:#g" start_ztncui.sh
sed -i "s#Default Password File Not Exists... Generating...#默认密码文件不存在... 正在生成...#g" start_ztncui.sh
sed -i "s#Cannot detect TLS Certs, Generating...#无法检测 TLS 证书，正在生成...#g" start_ztncui.sh

git clone https://github.com/key-networks/ztncui.git ztncui
...
sed -i "s#title: 'Easy setup of network',#title: '网络简易设置',#g" ztncui/src/controllers/networkController.js
...
sed -i "s#ZT 6plane (/80 routable for each device)#ZeroTier 6 PLANE(每个设备 /80 可路由)#g" ztncui/src/views/v6AssignMode.pug
